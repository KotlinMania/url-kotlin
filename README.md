...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`url`](https://crates.io/crates/url).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `url` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `url` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
