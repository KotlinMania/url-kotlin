# Claude Code Project Instructions — url-kotlin

## Project Overview

This is **url-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`url`](https://crates.io/crates/url).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/url/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/url>
