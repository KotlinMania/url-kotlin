...
@PublishedApi internal constructor(
    ...
    public fun slice(from: Position, to: Position): String {
        val start = when (from) {
            ...
            Position.BeforePort -> if (port != null) {
            ...
            Position.AfterPath -> {
                ...
                if (qs != null) qs else if (fs != null) fs else serialization.length
            }
    ...
    public fun host(): Host<String>? = when (host) {
    ...
    public fun domain(): String? = when (host) {
    ...
    public fun setUsername(username: String): Result<Unit> {
        ...
        usernameEnd = serialization.length - afterUsername.length +
            if (newUsernameEmpty && afterUsername.startsWith("@")) 1
            else if (!newUsernameEmpty && !afterUsername.startsWith("@") && !afterUsername.startsWith(":")) 1
            else 0
        hostStart += delta
        hostEnd += delta
        pathStart += delta
...
public data class UrlQuery(
    ...
    public fun appendPair(key: String, value: String): UrlQuery {
        ...
        u.setQuery(if (q.isEmpty()) "${encodeFormUrlencoded(key)}=${encodeFormUrlencoded(value)}"
            else "$q&${encodeFormUrlencoded(key)}=${encodeFormUrlencoded(value)}")
        return this
    }
...
internal val specialSchemes = setOf(
    "ftp", "file", "http", "https", "ws", "wss",
)

internal fun defaultPort(scheme: String): Int? = when (scheme.lowercase()) {
    "ftp" -> 21
    "http" -> 80
    "https" -> 443
    "ws" -> 80
