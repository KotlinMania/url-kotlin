...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Url.swiftmodule/`
...
//      `import Url` above would fail at compile time.
//
//   2. The static archive `libUrl.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Url" }` and
//      `flattenPackage = "io.github.kotlinmania.url"` configuration in
...
final class UrlExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Url swift module imported cleanly")
    }
}
